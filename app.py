...
with tab2:
    st.markdown("Click the microphone button to record, click again to stop.")
    st.markdown("---")
    ...
    if audio_file:
        st.audio(audio_file)
        with st.spinner("Recognizing speech..."):
            tmp2 = tempfile.NamedTemporaryFile(delete=False, suffix=audio_file.name[-4:])
            tmp2.write(audio_file.read())
            tmp2.close()
            wav_path2 = tmp2.name + "_conv.wav"
            subprocess.run(
                ["ffmpeg", "-y", "-i", tmp2.name, "-vn",
                 "-acodec", "pcm_s16le", "-ar", "16000", "-ac", "1", wav_path2],
                capture_output=True
            )
            recognizer2 = sr.Recognizer()
            try:
                with sr.AudioFile(wav_path2) as source:
                    audio_data2 = recognizer2.record(source)
                text2 = recognizer2.recognize_google(audio_data2, language="en-US")
                st.success(f'You said: "{text2}"')
                cleaned, intent, label, proba, response = run_pipeline(text2)
                st.session_state.history.append((text2, response, label, proba, cleaned))
                show_pipeline(text2, cleaned, label, proba, response)
                speak_response(response)
            except sr.UnknownValueError:
                st.error("Could not understand. Please try again.")
            except Exception as e:
                ...
                if os.path.exists(tmp2.name): os.unlink(tmp2.name)
                if os.path.exists(wav_path2): os.unlink(wav_path2)